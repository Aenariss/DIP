...
Logging times were recorded across a~sample of 100 web pages using the~\texttt{Measure-{\linebreak}Command}\footnote{\url{https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.utility/measure-command}} utility to assess the~performance of the~data collection process. The~measurement was performed on a~system with an~8-core \emph{Ryzen 7 9800X3D} CPU and \emph{32 GB} RAM. The~traffic logger was configured to remain on a~page for $7$ seconds, with a~timeout set to $10$ seconds. Of the~$100$ pages tested, $99$ loaded successfully, and the~entire collection process took 2,574 seconds. This corresponds to an~average of 25.74 seconds per page to complete all data collection, including overhead from browser automation and traffic capture.
...
\begin{table}[ht!]
 ...
 Firefox uBlock Origin     &   0 \\
        \hline
    \end{tabular}
\caption[Results of the~evaluation of the~number of request trees with blocked root node per tool]{Results of the~evaluation of the~number of trees with blocked root node per tool. The~results of Privacy Badger are unreliable as the~tool uses an algorithmic approach to block requests. Similarly, Avast Secure Browser and Brave browser blocked several root nodes incorrectly, as manual verification showed that in regular browsing session, those pages are not blocked.}
\label{blocked_root_nodes_table}
\end{table}

\subsection{Directly and Transitively Blocked Requests}
Table~\ref{direct_blocked_req_table} summarizes the number of requests blocked directly, transitively, and in total. Firefox's built-in content blocking functionality blocked the fewest requests by a considerable margin, indicating significantly weaker default performance. This outcome can be attributed to Firefox not blocking tracking content by default\footnote{\url{https://support.mozilla.org/en-US/kb/enhanced-tracking-protection-firefox-desktop\#w\_what-enhanced-tracking-protection-blocks}}.
...
Table~\ref{average_level_table} presents the number of blocked requests that triggered at least one follow-up request. Firefox's performance was again the weakest, consistent with previous results. In contrast, Ghostery for Firefox blocked the highest number of parent requests that caused further network activity, followed by Brave.

Avast Secure Browser and Adblock Plus blocked fewer parent requests compared to the best-performing tools, indicating a more conservative blocking strategy. For example, Adblock Plus blocked only \emph{41}~\% of the total number of requests blocked by Ghostery for Firefox but only \emph{31.7}~\% of the parent requests. Similarly, Avast Secure Browser blocked \emph{52}~\% of Firefox Ghostery's total blocked requests but only \emph{38}~\% of the parent requests. These results suggest that Adblock Plus and Avast Secure Browser allow more initiating requests to pass through, potentially resulting in more unwanted content being partially loaded.


\subsection{Average Request Block Level}
In addition to the number of blocked parent requests, Table~\ref{average_level_table} also presents the average request block level. As previously noted, Firefox blocked the fewest requests overall. Additionally, these requests tended to be deep within the request tree. Nevertheless, Firefox blocked at least one request in only \emph{81} out of \emph{937} collected trees, making this metric statistically less reliable for Firefox.
...
\setlength\itemsep{0em}
    ...
    \item \textit{Algorithmic Methods}: \textit{16,011}
    \item \textit{Crawl Fp Inspector}: \textit{7,460}
\end{itemize}

The results do not include API calls caused by unknown callers, as discussed in Subsection \ref{associating-fp-attempts}. Should such calls be included, the attempts in categories \emph{Browser Properties} and \emph{Crawl Fp Inspector} would remain the same, but the total attempts in \emph{Algorithmic Methods} would increase $13.5\times$, while the number of blocked \emph{Algorithmic Methods} by the tools would largely remain the same.


\subsection{Directly Blocked Calls To APIs Misused For Fingerprinting}
Table~\ref{fpd_direct_blocked_results} shows the number of potential fingerprinting attempts each tool blocked directly.
Ghostery for Firefox blocked the most API calls in the \textit{Browser Properties} category, closely followed by both versions of uBlock Origin. Both versions of uBlock Origin blocked the most \textit{Algorithmic Methods}, slightly outperforming Ghostery for Firefox. For \textit{Crawl Fp Inspector}, Brave blocked the most calls, surpassing all other tools in this category.
...
Table~\ref{fpd_attempts_blocked_transitively} presents the results for transitively blocked fingerprinting API calls.
Again, Ghost\-ery for Firefox blocked the most calls in the \textit{Browser Properties} category, with both versions of uBlock Origin following closely. For the \textit{Algorithmic Methods}, Ghostery for Firefox again performed best, slightly ahead of both uBlock Origin versions. In the \textit{Crawl Fp Inspector} category, Brave achieved the highest number of blocked calls.
...
Table~\ref{fpd_total_blocked_table} combines directly and transitively blocked fingerprinting API calls.
Ghostery for Firefox blocked the most API calls in the \textit{Browser Properties} category, outperforming its Chrome counterpart, with both uBlock Origin versions following closely. For the \textit{Algorithmic Methods}, Ghostery for Firefox again led, but both versions of uBlock Origin achieved nearly identical results. In the \textit{Crawl Fp Inspector} category, Brave blocked the most calls, followed by both versions of Ghostery, with the Chrome version slightly outperforming the Firefox version.

All evaluated extensions blocked an equal or greater number of \emph{Browser Properties} and \emph{Algorithmic Methods} calls in Firefox compared to Chrome. In contrast, for the \emph{Crawl Fp Inspector} category, all extensions in Chrome blocked an equal or greater number of calls than their Firefox counterparts.

Interestingly, while Ghostery for Firefox blocked \emph{24.5}~\% of all network requests, it reduced \emph{Browser Properties} API calls by \emph{66.6}~\%, and blocked \emph{39.2}~\% of \emph{Algorithmic Methods} calls. Its \emph{29.5}~\% blocking rate for \emph{Crawl Fp Inspector} was closest to its network request blocking rate. Similar patterns appeared for other tools, indicating that blocked requests were responsible for the majority of \emph{Browser Properties} API calls and an above-average amount of \emph{Algorithmic Methods} and \emph{Crawl Fp Inspector} API calls.
